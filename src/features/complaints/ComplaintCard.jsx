...
import { ChevronDown, ChevronsUp, ImageIcon, Star, Download, Bot, Trash2 } from 'lucide-react';

const ComplaintCard = ({ complaint, role, user, onUpdate, onSelect, isSelected, onDelete }) => { 
    ...
    const handleAdminUpdate = async (field, value) => {
        ...
        try {
            // 1. Prepare data for 'Resolved'
            if (field === 'status' && value === 'Resolved') {
                updateData.resolvedAt = serverTimestamp();
                
                // ✅ FIX: Safe Email Sending with Fallbacks
                if (complaint.userEmail) {
                    const templateParams = {
                        to_name: complaint.userName || 'Student', // Fallback prevents crash
                    ...
                    emailjs.send(emailjsConfig.serviceId, emailjsConfig.templateIdResolved, templateParams, emailjsConfig.publicKey)
                        .then(() => console.log('Resolved email sent.'))
                        .catch((err) => console.warn('EmailJS Warning (Non-critical):', err));
                }
            }
    ...
    const handleEscalate = async () => {
        ...
        try {
            ...
            createNotification({
                ...
                complaintId: complaint.id,
                type: 'escalation'
            }).catch(e => console.warn("Escalation notification failed", e));
