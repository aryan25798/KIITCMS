...
const MentorDashboard = () => {
    ...
    const handleRequestUpdate = async (id, status, studentId) => {
        try {
            ...
            // 2. If accepted, create the student-mentor connection record
            if (status === 'accepted') {
            ...
            // 3. Commit to Database FIRST (Critical Path)
            await batch.commit();

            // 4. Send Notification only if DB write succeeds (Non-Critical Path)
    ...
    const handleRemoveMentee = async (studentId) => {
        ...
        try {
            ...
            // 3. Find and delete associated 'accepted' requests to keep history clean
            const q = query(
    ...
    return (
        <div className="space-y-8">
                                    ...
                                    {pendingRequests.map(req => (
                                        <div key={req.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                            ...
                                            <div className="flex gap-2 flex-shrink-0">
                                                <button onClick={() => handleRequestUpdate(req.id, 'accepted', req.studentId)} className="px-3 py-1 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 transition-colors">Accept</button>
                                                <button onClick={() => handleRequestUpdate(req.id, 'rejected', req.studentId)} className="px-3 py-1 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 transition-colors">Decline</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    {acceptedRequests.map(req => (
                                        <div key={req.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50 flex justify-between items-center flex-wrap gap-4">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => onChat({ uid: req.studentId, name: req.studentName })} className="flex items-center gap-2 px-4 py-2 bg-cyan-500 text-white font-semibold rounded-lg hover:bg-cyan-600 transition-colors">
                                                    <MessageSquare size={18} /> Chat
                                                </button>
                                                <button onClick={() => setMenteeToRemove(req)} className="flex items-center gap-2 px-4 py-2 bg-red-600/80 text-white font-semibold rounded-lg hover:bg-red-700 transition-colors">
                                        ...
                                        <div key={app.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50">
                                            ...
                                            {app.status === 'Pending' && (
                                                <div className="flex gap-2 mt-4">
                                                    <button onClick={() => handleLeaveUpdate(app.id, 'Approved', app.userId)} className="px-3 py-1 bg-green-600 text-white rounded-md text-sm hover:bg-green-700 transition-colors">Approve</button>
                                                    <button onClick={() => handleLeaveUpdate(app.id, 'Rejected', app.userId)} className="px-3 py-1 bg-red-600 text-white rounded-md text-sm hover:bg-red-700 transition-colors">Reject</button>
                                                </div>
                                            )}
                                        </div>
                                    ))}
            ...
            {/* Remove Mentee Confirmation Modal */}
            {menteeToRemove && (
